...
//NOT PRECISE! Quick and Dirty Measurements
	...
	while(1){	
		...
		while (nPacketsInQueue > 0) {
			...
			if ((pl != NULL )&&(pl->data != NULL)){
				...
				x1 = LSM9DS0_SENSITIVITY_MULTIPLIER_X * (double) x + LSM9SS0_OFFSET_X;							//Converts to real g-values
				y1 = LSM9DS0_SENSITIVITY_MULTIPLIER_Y * (double) y + LSM9SS0_OFFSET_Y;
				z1 = LSM9DS0_SENSITIVITY_MULTIPLIER_Z * (double) z + LSM9SS0_OFFSET_Z;

				//Creates the vector sum, representing the overall acceleration minus normal earth acceleration
				A = sqrt( x1*x1+y1*y1+z1*z1)-1;	

				//Prints out the acceleration in collums    |total |X acc |Y acc |Z acc |
				printf("Total Acc: %.3f \tX: %.3f \tY: %.3f \tZ: %.3f\t",A, x1, y1, z1);	 
				

				//A simple fall detection algorithm
				if (A>3.6){																
					printf("FALL ALERT!!!\n");
				}
				else{
