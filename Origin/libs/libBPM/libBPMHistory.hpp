/**
 * \file libBPMHistory.hpp
 * \brief Generic History Template class that will track history of a data stream
 * 2009-2013 P.Kindt <pkindt@directbox.com>
 * Tools to manage the history of a signal, i.e. its behaviour
 * during the last frames.
 *
 * Indexes (i.e. everywhere the type t_hist-index occurs) have the follwing meaning:
 * zero is the last element added, every whalue > 0 means how many elements in history
 * you want to go back.
 * Example:
 * 	libBPMHistory <char*> hist;
 *	hist.add("first");
 * 	hist.add("2nd");
 *	hist.add("3rd");
 ...
 ****************************************************************************/
#ifndef LIBBPM_HISTORY_H_
#define LIBBPM_HISTORY_H_
...
template <class T> class libBPMHistory{
public:
...
public:
	...
	///constructor
	libBPMHistory(unsigned int size);

	///copy-constructor
	libBPMHistory(const libBPMHistory<T>& hm);
	...
	///calculate arithmethic mean value. "Simple" because libBPMHistoryManager's averageing methods can do much more
	T getSimpleAverage(t_hist_index IndexFrom, t_hist_index IndexTo);

	///calculate median value. "Simple" because libBPMHistoryManager's averageing methods can do much more
...
template <class T> void libBPMHistory<T>::inkr(){
 ...
 */
template <class T> unsigned int libBPMHistory<T>::getPos(t_hist_index index) const{
	unsigned int rv;
	if(index > position){
...
template <class T> libBPMHistory<T>::libBPMHistory(unsigned int size): RingBuf(size){
	position = 0;
	BiggestIndex=0;
	empty = true;
}

template <class T> libBPMHistory<T>::libBPMHistory(const libBPMHistory<T>& hm): RingBuf(hm.RingBuf){
...
///is buffer completely empty?
template <class T> bool libBPMHistory<T>::isEmpty() const{
	return empty;
}

///is buffer complete filled (are there as many elements added as there are free places)?
template <class T> bool libBPMHistory<T>::isFilled() const{
	if(BiggestIndex == RingBuf.size()-1) return true;
	return false;
}

///how many elements are there in the buffer? (if filled once, it will be size allways!)
template <class T> unsigned int libBPMHistory<T>::getNrOfElements() const{
	if(isEmpty()) return 0;
	return BiggestIndex + 1;
}

///returns  biggest array index
template <class T> unsigned int libBPMHistory<T>::getBiggestIndex() const{
	return BiggestIndex;
}

///gets number of entries allocated - this is the size
template <class T> unsigned int libBPMHistory<T>::getSize() const{
	return RingBuf.size();
}

///add an Element to history's ringbuffer
template <class T> void libBPMHistory<T>::add(T element){
	inkr();
 ...
 * !Handle with care!
 */
template <class T> void libBPMHistory<T>::shiftPositionCursor(int NewIndex){
	if(NewIndex < 0){
		NewIndex = 0;
		empty = true;
	}else{
	...
	if(NewIndex > (RingBuf.size() - 1)){
		DEBUGMSG("libBPMHistory<T>::ShiftPositionCursor(): Invalid NewIndex; NewIndex="<<NewIndex<<", size="<<RingBuf.size());
...
///same as add
template <class T> void libBPMHistory<T>::operator+=(const T& element){
	add(element);
}
 ...
 */
template <class T> T& libBPMHistory<T>::get(unsigned int index){
 ...
 * remove element from buffer (like used to happen on some stack implementations)
 */
template <class T> T libBPMHistory<T>::get(unsigned int index) const{
	return RingBuf[getPos(index)];
}

///same as get
template <class T> T libBPMHistory<T>::operator[](unsigned int index) const{
	return get(index);
}

///sum up history's elements from index IndexFrom to index IndexTo
template <class T> T libBPMHistory<T>::getSum(t_hist_index IndexFrom, t_hist_index IndexTo) const{
	if((IndexTo > BiggestIndex)||(IndexFrom > IndexTo)){
		DEBUGMSG("libBPMHistory::GetSum(): Invalid Index IndexFrom="<<IndexFrom<<" IndexTo="<<IndexTo);
 ...
 *'simple' because libBPMHistoryManager's averaging functions can do much more (median,...)
 */
template <class T> T libBPMHistory<T>::getSimpleAverage(t_hist_index IndexFrom, t_hist_index IndexTo){
	///avoid bad indexes
	if((IndexTo > BiggestIndex)||(IndexFrom > IndexTo)){
		DEBUGMSG("libBPMHistory::GetSimpleAverage(): Invalid Index IndexFrom="<<IndexFrom<<" IndexTo="<<IndexTo);
		DEBUGMSG("This is a software bug. Please change the source... bye");
		exit(1);
 ...
 */
template <class T> T libBPMHistory<T>::getSimpleMedian(t_hist_index IndexFrom, t_hist_index IndexTo){
	///avoid bad indexes
	if((IndexFrom > BiggestIndex)||(IndexFrom > IndexTo)){
		DEBUGMSG("libBPMHistory::GetMedian(): Invalid Index IndexFrom="<<IndexFrom<<" IndexTo="<<IndexTo);
		DEBUGMSG("This is a software bug. Fix and recompile! bye...");
		exit(1);
	}
	if(IndexTo > libBPMHistory<T>::BiggestIndex){
		IndexTo = libBPMHistory<T>::BiggestIndex;
	}

	if(IndexFrom > IndexTo){
		IndexFrom = IndexTo;
		DEBUGMSG("libBPMHistorymanager::GetMedian(): IndexFrom > IndexTo: had to adjst: IndexFrom="<<IndexFrom<<"  IndexTo="<<IndexTo);
	}
	...
	///efficiently => build array first, use quicksort on it after that
	 ...
	 for(int cnt = 0; cnt < (IndexTo - IndexFrom + 1); cnt++){
	    	 hektor[cnt] = libBPMHistory<T>::get(IndexFrom + cnt);
	 }
	...
	* In case of two equal extrema, the smallest index is returned.
	*/
	template <class T> t_hist_index libBPMHistory<T>::getIndexOfExtremum(histExtremumType ExtrType, T* value,unsigned int AvgWindowLength = 0, bool StopAtValue=false, T StopValue = (T)0){
	T ExtremumValue, TempValue;
	bool ForceNoAvg = false;
	if(AvgWindowLength > getNrOfElements()){
		DEBUGMSG("libBPMHistory::GetIndexOfExtremum(): Averaging Frame Window exceeds Number of Elements in History");
...
///clear history
template <class T> void libBPMHistory<T>::clear(){
	position = 0;
	BiggestIndex=0;
	empty = true;
...
template <class T> void libBPMHistory<T>::print(){
	cout<<"----[libBPMHistory Instance "<<this<<"]---------"<<endl;
